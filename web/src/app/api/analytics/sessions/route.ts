...
import { getDemoSessionsData } from '@/lib/demoWorkspaceData';
import { fetchAnalyticsSessionsData } from '@/lib/analyticsSubpageServer';
...
export async function GET(req: Request) {
    ...
    if (context.isDemoWorkspace) {
        return NextResponse.json(getDemoSessionsData(), { headers: { 'Cache-Control': 'private, max-age=60, stale-while-revalidate=30' } });
    }
    ...
    const cacheKey = `ga:analytics-sessions:${context.userId}:${context.propertyId}:${range}`;
    const data = await cachedFetch(
        cacheKey,
        CACHE_TTL.DASHBOARD_DATA,
        () => fetchAnalyticsSessionsData(context.accessToken!, context.propertyId!, range),
    );
