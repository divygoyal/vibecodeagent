...
export async function GET(req: Request) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error("Admin API error listing properties:", response.status, response.statusText, errorText);
            return NextResponse.json({ error: "Failed to list properties", details: errorText }, { status: 500 });
        }
