...
import { getDemoPagesData } from '@/lib/demoWorkspaceData';
import { fetchAnalyticsPagesData } from '@/lib/analyticsSubpageServer';
...
export async function GET(req: Request) {
    ...
    if (context.isDemoWorkspace) {
        return NextResponse.json(getDemoPagesData(), { headers: { 'Cache-Control': 'private, max-age=60, stale-while-revalidate=30' } });
    }
    ...
    const cacheKey = `ga:analytics-pages:${context.userId}:${context.propertyId}:${range}`;
    const data = await cachedFetch(
        cacheKey,
        CACHE_TTL.DASHBOARD_DATA,
        () => fetchAnalyticsPagesData(context.accessToken!, context.propertyId!, range),
    );
