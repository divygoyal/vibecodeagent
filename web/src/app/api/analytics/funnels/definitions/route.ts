...
import { getDemoFunnelDefinitions } from '@/lib/demoWorkspaceData';
...
export async function GET(req: Request) {
    ...
    if (isDemoRequest(req)) {
        return NextResponse.json(getDemoFunnelDefinitions(), { headers: { 'Cache-Control': 'private, max-age=120, stale-while-revalidate=60' } });
    }
...
export async function POST(req: Request) {
    ...
    if (isDemoRequest(req)) {
        return NextResponse.json({ error: 'Funnel creation is disabled while demo data is active' }, { status: 403 });
    }
