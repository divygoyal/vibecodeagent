...
export function ChatHeader({
    ...
    return (
        <div className="px-4 py-3 border-b border-[var(--card-border)] flex items-center justify-between gap-2 bg-[var(--header-bg)] safe-area-top">
            <div className="flex items-center gap-2.5 min-w-0 flex-1">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-400 to-cyan-400 flex items-center justify-center relative flex-shrink-0">
                ...
                <div className="min-w-0 flex-1">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="flex items-center gap-1 text-[10px] text-zinc-500 hover:text-zinc-300 transition-colors mt-0.5 max-w-full"
                            aria-label="Switch website context for the chat"
                            aria-expanded={showSiteDropdown}
                            aria-haspopup="listbox"
                        >
                            <Globe className="w-2.5 h-2.5 flex-shrink-0" />
                            <span className="max-w-[120px] sm:max-w-[160px] truncate">{currentSiteLabel}</span>
                            <ChevronDown className={`w-2.5 h-2.5 flex-shrink-0 transition-transform ${showSiteDropdown ? 'rotate-180' : ''}`} />
                        </button>
                        {showSiteDropdown && (
                            <>
                                <div className="fixed inset-0 z-40" onClick={() => setShowSiteDropdown(false)} />
                                <div className="absolute left-0 top-full mt-1 z-50 bg-[var(--dropdown-bg)] border border-[var(--card-border)] rounded-xl shadow-2xl shadow-black/80 py-1 min-w-[200px] max-w-[calc(100vw-2rem)] max-h-[200px] overflow-y-auto">
                                    {allSites.length === 0 ? (
