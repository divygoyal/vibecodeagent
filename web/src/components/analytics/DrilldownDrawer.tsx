        ...
        <AnimatePresence>
            {open && (
                <>
                    ...
                    >
                        {/* Header */}
                        <div className="sticky top-0 bg-[var(--sidebar-bg)] backdrop-blur-md border-b border-[var(--divider)] px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between z-10">
                            <div className="min-w-0 flex-1">
                                <p className="text-[10px] text-zinc-500 uppercase tracking-wider font-medium">Drilldown</p>
                                <h2 className="text-base sm:text-lg font-bold text-[var(--text-primary)] mt-0.5 truncate">{data.value}</h2>
                            </div>
                            <button onClick={onClose} className="p-2.5 sm:p-2 rounded-lg hover:bg-white/[0.06] text-zinc-500 hover:text-white transition ml-2 shrink-0 min-h-[44px] min-w-[44px] flex items-center justify-center">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                            ...
                            {data.metrics && data.metrics.length > 0 && (
                                <div className="grid grid-cols-2 gap-2 sm:gap-3">
                                    {data.metrics.map((m, i) => (
                                        <div key={i} className="bg-white/[0.03] border border-white/[0.04] rounded-xl p-3 sm:p-3.5">
                                            <p className="text-[10px] text-zinc-500 uppercase tracking-wider font-medium">{m.label}</p>
                                            <p className="text-lg sm:text-xl font-bold text-white mt-1 tabular-nums">{typeof m.value === 'number' ? m.value.toLocaleString() : m.value}</p>
                            ...
                            <button
                                ...
                                className="w-full py-3 sm:py-2.5 text-sm sm:text-xs font-semibold text-white bg-gradient-to-r from-blue-600 to-cyan-500 rounded-xl hover:opacity-90 transition min-h-[44px]"
                            >
