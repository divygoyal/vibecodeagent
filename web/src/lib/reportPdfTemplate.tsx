...
function PageHeader({ siteUrl }: { siteUrl: string }) {
    return (
        <View style={s.header}>
            <Text style={s.headerLogo}>{BRAND_NAME}</Text>
...
function CoverPage({ siteUrl, period, hasGa4 }: { siteUrl: string; period: ReportPeriod; hasGa4: boolean }) {
    ...
    return (
        <Page size="A4" style={[s.page, { justifyContent: 'center', alignItems: 'center' }]}>
            ...
            <Text style={{ fontSize: 32, fontFamily: 'Helvetica-Bold', color: C.emerald, marginBottom: 4 }}>{BRAND_NAME}</Text>
    ...
    return (
        <Page size="A4" style={[s.page, { justifyContent: 'center', alignItems: 'center' }]}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center', flex: 1 }}>
                <Text style={{ fontSize: 28, fontFamily: 'Helvetica-Bold', color: C.emerald, marginBottom: 16 }}>{BRAND_NAME}</Text>
            ...
            <View style={{ position: 'absolute', bottom: 24, left: 40, right: 40, alignItems: 'center' }}>
                <Text style={{ fontSize: 7, color: C.textLight }}>Generated by ${BRAND_NAME} - ${SITE_HOST} - 2026 ${BRAND_NAME}. All rights reserved.</Text>
            </View>
        </Page>
    );
}
...
export function ReportDocument({ analysis, gemini, period, siteUrl }: ReportDocumentProps) {
    ...
    return (
        <Document
            title={`${BRAND_NAME} ${period.type === 'weekly' ? 'Weekly' : 'Monthly'} Deep Report - ${siteUrl}`}
            author={`${BRAND_NAME}`}
