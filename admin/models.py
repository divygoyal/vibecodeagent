...
class SponsorshipListing(Base):
    """A publisher's own pricing for their leaderboard entry's sponsor slot.

    017_add_sponsorship_marketplace.sql.

    A MISSING ROW MEANS "use the computed auto price". Every verified entry is
    priced and bookable from the first deploy via
    `web/src/lib/sponsorshipPricing.ts`; this table only exists so a publisher
    who disagrees with the suggestion can override it. Never insert a row just
    to mirror the computed price — an absent row is the desired steady state.

    `accepting_requests = False` keeps the listing visible (it still ranks and
    still earns the backlink) but marks it unbookable. That is the property
    that makes an unsold listing cost nothing to leave up, which is the whole
    reason this is a booking marketplace rather than an ad network.

    `auto_price_cents_at_save` records what the model suggested at the moment
    the publisher saved, so we can answer "do publishers price above or below
    our formula?" without a backfill later.
    """
    __tablename__ = "sponsorship_listings"

    id = Column(Integer, primary_key=True)
    entry_id = Column(Integer, nullable=False, index=True)
    price_cents = Column(Integer)  # NULL = inherit the computed price
    floor_cents = Column(Integer)  # advisory only; nothing enforces it
    accepting_requests = Column(Boolean, nullable=False, default=True)
    placement_note = Column(Text)
    auto_price_cents_at_save = Column(Integer)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)

    __table_args__ = (
        UniqueConstraint("entry_id", name="uq_sponsorship_listing_entry"),
    )


class SponsorshipRequest(Base):
    """One advertiser submission. May cover many sites (see the items table).

    017_add_sponsorship_marketplace.sql.

    Pre-payment: a qualified lead with a price snapshot, not an order. There is
    no money anywhere in this table. `users.credits` is the AI chat counter and
    must not be reused for sponsorship money; when real payments land they get
    their own append-only ledger where a balance is always SUM(amount_cents).

    `source` is 'profile' (one site, from /leaderboard/{id}) or 'budget' (many
    sites, from the /advertise planner). Keeping them distinguishable is how we
    learn which surface actually produces buyers.
    """
    __tablename__ = "sponsorship_requests"

    id = Column(Integer, primary_key=True)
    advertiser_email = Column(String(255), nullable=False, index=True)
    advertiser_name = Column(String(120))
    advertiser_site = Column(String(500))
    source = Column(String(20), nullable=False, default="profile")
    source_path = Column(String(255))
    budget_cents = Column(Integer)  # what the advertiser said they had
    quoted_total_cents = Column(Integer, nullable=False, default=0)
    site_count = Column(Integer, nullable=False, default=0)
    message = Column(Text)
    # new | contacted | partially_won | won | lost — rolled up from the items
...
class SponsorshipRequestItem(Base):
    """One site inside a request. This row is the conversion record.

    017_add_sponsorship_marketplace.sql.

    The 30-day gate is "3 paid sponsorships at any price", and this is where
    that gets counted — hence the funnel timestamps existing from day one
    rather than being retro-fitted:

        created_at    request arrived
        notified_at   both parties emailed
        responded_at  publisher first replied
        paid_at       operator confirmed money received  <- THE GATE
        live_at       creative actually visible on the site

    `paid_amount_cents` is a record of a manual invoice settled outside the
    product. It is not a balance and nothing debits it.
    """
    __tablename__ = "sponsorship_request_items"

    id = Column(Integer, primary_key=True)
    request_id = Column(Integer, nullable=False, index=True)
    entry_id = Column(Integer, nullable=False, index=True)
    # Denormalised: manual fulfilment means humans read these rows, and they
    # must still make sense if the entry is renamed or deactivated later.
    entry_name = Column(String(150))
    entry_domain = Column(String(255))
    price_cents_at_request = Column(Integer, nullable=False)
    price_source = Column(String(20), nullable=False, default="computed")
    ...
    paid_amount_cents = Column(Integer)
